    ...
    def authorize_mcp_tool(
        ...
        if tool_name == "search_documents":
            ...
            if len(query) > max_len:
                ...
                return decision
            blocked_keywords = [
                str(keyword).lower() for keyword in tool_policy.get("blocked_keywords", [])
            ]
            matched = next((word for word in blocked_keywords if word in query.lower()), None)
            if matched:
                decision = GovernanceDecision(
                    verdict=GovernanceVerdict.DENY,
                    reason=f"Từ khóa bị chặn bởi governance: {matched}",
                    actor_id=actor_id,
                    connection_type=ConnectionType.MCP,
                    resource=f"mcp:research-tools/{tool_name}",
                )
